...
**Committed by design — do not flag these.** Several `assets/data/*.json`
files are generated by scheduled GitHub Actions and committed on every run:
`articles-index.json`, `ga-metrics.json`, `ga-hourly.json`,
`ga-hourly-archive.json`, `club-news.json`, `club-news-failures.json`, plus
the hand-maintained `clubs-meta.json`, `competitions-meta.json`,
