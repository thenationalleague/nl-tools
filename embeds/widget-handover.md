...
- **Submit interrupts only when rows are unset.** Every row set → it just writes. Predictions stay editable until each match's cutoff, so confirming a fully-set submit is ceremony. If any row was never tapped, a review modal lists every fixture at the exact scoreline it will be saved at — crests + clubs-meta short names, home side right-aligned into the scoreline, away side left-aligned out of it, with a matchday + division header — and the untouched ones are muted, tint their score chip amber and carry a "not set" flag. That case earns the interruption because an accidental 0–0 is indistinguishable from a deliberate one afterwards.
- There is deliberately **no "X/12 set" counter**: it counted rows the fan had *tapped*, so a deliberate 0–0 read as "11/12 set" and then submitted 12. Don't reinstate it.
