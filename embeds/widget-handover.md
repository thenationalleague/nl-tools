...
- **Bulk submit is WYSIWYG** (`#…submitbar`, "Review & submit N predictions") — always enabled; every open match is written at its displayed value, which makes 0–0 a first-class pick (just leave the row alone). Untouched rows stay editable per-row until each match's own 60-min cutoff. One RTDB `.update()` writes all rows in one batch.
- **Submit always opens a review modal** listing every fixture at the exact scoreline it will be saved at — crests + clubs-meta short names, home side right-aligned into the scoreline, away side left-aligned out of it, with a matchday + division header. Never-tapped rows are muted, tint their score chip amber and carry a "not set" flag. There is deliberately **no "X/12 set" counter**: it counted rows the fan had *tapped*, so a deliberate 0–0 read as "11/12 set" and then submitted 12. The list answers the question the counter was answering badly. (This also replaced an older confirm that only fired when *every* fixture was an untouched 0–0 — one narrow case of the same problem.)
