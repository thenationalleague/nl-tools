...
(function () {
  ...
  function mount() {
    ...
    (function () {
      ...
      // 0–0 pick alone read as "11/12 set" and then submitted 12. The review
      // modal replaces it: it shows exactly what will be saved, which is the
      // question the counter was badly answering.
      function renderSubmitbar() {
        var await_ids = awaitingIds();
        if (!await_ids.length) { submitb.innerHTML = ''; return; }
        submitb.innerHTML =
          '<div class="nlsp__submitbar">' +
            '<button class="nlsp__btn" id="nlsp-submit">Review &amp; submit ' +
      ...
      function submitAllAwaiting() {
        ...
        // Always review before writing. Every submit is WYSIWYG — untouched rows
        // go in at 0–0 — so the fan sees the full list exactly as it will be
        // saved, with the never-tapped rows flagged, and confirms that.
        showConfirm({
          title:    'Confirm your predictions',
          message:  untouched > 0
            ? untouched + ' of ' + ids.length + ' ' + (untouched === 1 ? 'match has' : 'matches have') +
              ' not been set and will be submitted as 0–0.'
            : 'These will be saved for this matchday. You can still edit them until each match locks.',
