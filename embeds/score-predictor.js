...
(function () {
  ...
  function mount() {
    ...
    (function () {
      ...
      function flashSaveTick() { setBannerOK('Saved'); }
      ...
      function rowHTML(m, now, hero) {
        ...
        if (s === 'future') {
          ...
          meta = '<span class="nlsp__voidlbl">Postponed</span><span class="nlsp__pickline" style="opacity:.7;">Prediction voided</span>';
          rowMod = ' is-void';
        } else if (s === 'abandoned') {
          meta = '<span class="nlsp__voidlbl">Abandoned</span><span class="nlsp__pickline" style="opacity:.7;">Prediction voided</span>';
          rowMod = ' is-void';
        ...
        function scoreCell(side) {
          ...
          if (s === 'future' || s === 'postponed' || s === 'abandoned' || s === 'unresolved') {
      ...
      // 0–0 pick alone read as "11/12 set" and then submitted 12. The unset-rows
      // review (see submitAllAwaiting) answers that question properly, and only
      // when it actually needs asking.
      function renderSubmitbar() {
        var await_ids = awaitingIds();
        if (!await_ids.length) { submitb.innerHTML = ''; return; }
        submitb.innerHTML =
      ...
      // The review list — every fixture going in, at the value it will be saved
      // at. Untouched rows are shown and flagged rather than hidden: they submit
      // as 0–0, so that is exactly what the fan needs to see before confirming.
      // Laid out like a results list: home name right-aligned into the score,
      // away name left-aligned out of it, crests flanking the scoreline.
      function submitReviewHTML(ids) {
        var byId = {};
        state.matches.forEach(function (m) { byId[m.id] = m; });

        var side = function (t, isHome) {
          var name = $h(clubShort(t.teamID, t.shortName || t.name));
          var crest = crestImg(t.crest, t.name || t.shortName || '', 20);
          return '<span class="nlsp__reviewteam is-' + (isHome ? 'home' : 'away') + '">' +
            (isHome ? '<span class="nlsp__reviewname">' + name + '</span>' + crest
                    : crest + '<span class="nlsp__reviewname">' + name + '</span>') +
          '</span>';
        };
        ...
        var rows = ids.map(function (mid) {
          ...
          var a = m.attributes;
          var d = state.drafts[mid];
          var set = !!(d && d.home != null && d.away != null);
          var hs = set ? d.home : 0, as = set ? d.away : 0;
          return '<div class="nlsp__reviewrow' + (set ? '' : ' is-untouched') + '">' +
            side(a.homeTeam, true) +
            '<span class="nlsp__reviewmid">' +
              '<span class="nlsp__reviewscore">' + hs + '–' + as + '</span>' +
              (set ? '' : '<span class="nlsp__reviewnote">not set</span>') +
            '</span>' +
            side(a.awayTeam, false) +
          '</div>';
        }).join('');

        // Matchday + division header. Every fixture in the view is in the user's
        // own division (recomputeMatchday filters by comp), so one line covers
        // the whole list.
        var key = currentMatchdayKey();
        var dateLabel = key
          ...
          : '';
        var head = '<div class="nlsp__reviewhead">' + $h(dateLabel) +
          (dateLabel ? ' · ' : '') + $h(compName(userCompId())) + '</div>';

        return head + '<div class="nlsp__review">' + rows + '</div>';
      }

      // Submit is WYSIWYG: every open match goes in at its displayed value, so a
      // 0–0 pick is just... left at 0–0 (no stepper gymnastics). Untouched rows
      // are called out in a confirm first so nobody logs twelve 0–0s by accident.
      function submitAllAwaiting() {
        var ids = awaitingIds();
        if (!ids.length) return;
        var updates = {}, untouched = 0;
        ...
        // Only interrupt when there's something to warn about. Predictions stay
        // editable until each match's own cutoff, so confirming a submit where
        // every row was deliberately set is pure ceremony — it just writes.
        // Untouched rows are the exception: they go in at 0–0 and afterwards look
        // identical to a deliberate 0–0, so those get the review list first.
        if (untouched === 0) { doWrite(); return; }

        showConfirm({
          title:    'Some matches not set',
          message:  untouched + ' of ' + ids.length + ' ' + (untouched === 1 ? 'match has' : 'matches have') +
                    ' not been set and will be submitted as 0–0.',
          bodyHtml: submitReviewHTML(ids),
          confirm:  'Submit all',
          cancel:   'Go back'
        }).then(function (ok) { if (ok) doWrite(); });
      }
        ...
        return new Promise(function (resolve) {
          modalEl.innerHTML =
            '<div class="nlsp__modal-card" role="dialog" aria-modal="true">' +
              ...
              // bodyHtml is built internally (submitReviewHTML) from already-
              // escaped parts — never from user input.
              (opts.bodyHtml || '') +
        ...
        if (btn) btn.addEventListener('click', function () {
          showConfirm({
            ...
            fbDb.ref('predictions/' + state.user.id + '/' + currentMatchdayKey()).update(nulls)
              .then(function () {
                state.drafts = {}; state.editing = {};
                setBannerOK('All predictions reset.');
              })
              .catch(function (err) {
      ...
      function seasonMonths() {
        var seen = {};
        (state.allMatches || []).forEach(function (m) {
          var mo = monthOfMatchday(matchMatchdayKey(m));
          if (mo) seen[mo] = true;
        });
        return Object.keys(seen).sort();
      }
      ...
      // Every matchday key across ALL competitions (the leaderboard spans fans of
      // every division, unlike the datebar which is scoped to the user's comp).
      ...
      function filterControlsHTML() {
        ...
        var html = '<div class="nlsp__tablefilters">' +
          '<div class="nlsp__chipgroup" id="nlsp-lb-scope" role="group" aria-label="Time period">' +
            ...
            chip('Month', 'data-s="month"', state.lbScope === 'month') +
            chip('Matchday', 'data-s="day"', state.lbScope === 'day') +
          '</div>';
        if (state.lbScope === 'month') {
          html += '<select id="nlsp-lb-month" aria-label="Month">' +
            seasonMonths().map(function (mo) {
