...
(function () {
  ...
  function mount() {
    ...
    (function () {
      ...
      // Of those awaiting, count how many have a complete (home + away) draft
      function awaitingReadyCount() {
        return awaitingIds().reduce(function (n, mid) {
          var d = state.drafts[mid];
          return n + ((d && d.home != null && d.away != null) ? 1 : 0);
        }, 0);
      }
      ...
      function renderSubmitbar() {
        ...
        var ready = awaitingReadyCount();
        submitb.innerHTML =
          '<div class="nlsp__submitbar">' +
            '<div class="nlsp__submitmsg"><b>' + ready + '/' + await_ids.length + '</b> set</div>' +
            '<button class="nlsp__btn" id="nlsp-submit" style="margin-left:auto;">Submit predictions</button>' +
          '</div>';
      ...
      function submitAllAwaiting() {
        ...
        // 0–0 is a legitimate pick, so partial submits go through silently even
        // with untouched rows (they stay editable until their own cutoffs). The
        // confirm only fires when EVERY fixture going in is an untouched 0–0
        // (and there's more than one) — the "tapped Submit without doing
        // anything" case.
        if (untouched === ids.length && ids.length > 1) {
          showConfirm({
            title:   'Submit as shown?',
            message: 'All ' + ids.length + ' matches are at 0–0 and will be submitted that way.',
            confirm: 'Submit',
            cancel:  'Go back'
