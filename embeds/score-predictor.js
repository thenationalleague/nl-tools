      ...
      function rowHTML(m, now, hero) {
        ...
        function scoreCell(side) {
          ...
          /* Called off: the printed-grid letter where the score would be — P for
             ...
             The letter sits on the home line only, with the away cell left as a
             spacer. The two team lines stack, so one letter to the right of the
             fixture is the newspaper treatment; a letter on each line would read
             as two separate outcomes. The full wording lives in the title and
             aria-label, so nothing is lost to a screen reader or a hover. */
          if (s === 'postponed' || s === 'abandoned') {
            if (side === 'away') return '<div class="' + cls + ' is-void" aria-hidden="true"></div>';
