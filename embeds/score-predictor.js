      ...
      function submitReviewHTML(ids) {
        ...
        var rows = ids.map(function (mid) {
          ...
          return '<div class="nlsp__reviewrow' + (set ? '' : ' is-untouched') + '">' +
            '<span class="nlsp__reviewcode">' + $h(clubCode(a.homeTeam.teamID, a.homeTeam.shortName || a.homeTeam.name)) + '</span>' +
            '<span class="nlsp__reviewscore">' + hs + '–' + as + '</span>' +
            '<span class="nlsp__reviewcode">' + $h(clubCode(a.awayTeam.teamID, a.awayTeam.shortName || a.awayTeam.name)) + '</span>' +
            '<span class="nlsp__reviewnote">' + (set ? '' : 'not set') + '</span>' +
          '</div>';
        }).join('');
        return '<div class="nlsp__review">' + rows + '</div>';
      }
