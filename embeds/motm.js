...
(function () {
  ...
  function mount() {
    ...
    (function () {
      ...
      // than offering navigation — there is nothing else to look at — and
      // carries the club's next few fixtures so "come back" has a date on it.
      function windowCardHTML() {
        var reg = state.registration;
        var club = reg ? clubShort(reg.teamId, reg.teamName) : 'your club';
        var upcoming = nextFixtures(3);

        var tail = upcoming.length
          ? 'Come back after the next game.'
          : 'That\'s the last of your fixtures for now — nominations reopen when the new season does.';

        var list = '';
        if (upcoming.length) {
          list =
            '<div class="nlsm__fixtures">' +
              '<div class="nlsm__fixtureshead">Next ' +
                (upcoming.length === 1 ? 'fixture' : upcoming.length + ' fixtures') +
              '</div>' +
              upcoming.map(function (m) {
                ...
                return '<div class="nlsm__fixture">' +
                  crestImg(opp.crest, opp.name || opp.shortName || '', 24) +
