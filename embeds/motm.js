...
(function () {
  ...
  function mount() {
    ...
    (function () {
      ...
      // Voting opens at FULL TIME and closes at 23:59:59 UK time on the day after
      // the match's own matchday — a Saturday game stays open until Sunday night
      // whatever time it kicked off. A match that never reaches full time never
      // opens for voting at all.
      //
      // It was KO+24h, which made the deadline a function of kick-off time: a
      // 12:30 Saturday game shut at lunchtime on Sunday and a 17:20 one at
      // twenty past five, so two games on the same matchday closed five hours
      // apart and neither closed at a time anybody could remember. A matchday
      // has one deadline now.
      ...
      function fmtBSTWeekday(d) {
        return d ? d.toLocaleDateString('en-GB', { weekday: 'long', timeZone: 'Europe/London' }) : '';
      }
      function relSoon(ms) {
        if (ms <= 0) return 'now';
      ...
      var state = {
        ...
        registration: null,
        windows: null,      // { matchId: closeEpoch } — the server's own gate; null until loaded
        windowsFailed: false, // could not read it; fall back rather than lock everyone out
        ...
        loadedSeasonId: null,  // season the currently-held fixtures were fetched for
      ...
      function setSimFixed(d) {
        state.sim.fixed = d;
        // Moving the sim clock across 1 July changes which season's fixtures we
        // should be looking at, so reload them rather than showing the wrong set.
        if (activeSeasonId() !== state.loadedSeasonId) { reloadFixtures(); return; }
        recomputeMatch(); renderAll();
      }
      function clearSimFixed() { setSimFixed(null); }
      function reloadFixtures() {
        showBanner('Loading ' + activeSeasonId() + '/' + String(activeSeasonId() + 1).slice(2) + ' fixtures\u2026');
        fetchFixtures().then(function (ms) {
          state.allMatches = ms;
          state.loadedSeasonId = activeSeasonId();
          teamCompCache = null;
          state.players = {};   // lineups are per match, but the match set changed
          hideBanner();
          // The club is resolved against the season's teams, so re-resolve it.
          state.registration = resolveTeamFromSso() || state.registration;
          recomputeMatch(); renderAll();
        }).catch(function (err) {
          showBanner('Could not load fixtures: ' + (err && err.message ? err.message : err), 'err');
        });
      }
      function renderAll() {
      ...
      // for voting at a time (full time, until the end of the following day), so the widget
      ...
      /* The inverse of bstDateOf: a London wall-clock midnight as an instant.
         ...
         correcting by the offset London actually had then lands on the real one.

         Twice, because the first correction can itself land the other side of a
         DST boundary — the last Sunday in October, within an hour of 02:00, is
         the case a single pass gets wrong. Cheap enough to just always do. */
      function londonOffsetMs(t) {
        var f = new Intl.DateTimeFormat('en-GB', {
          timeZone: 'Europe/London', hour12: false,
          year: 'numeric', month: '2-digit', day: '2-digit',
          hour: '2-digit', minute: '2-digit', second: '2-digit'
        }).formatToParts(new Date(t)).reduce(function (a, p) { a[p.type] = p.value; return a; }, {});
        return Date.UTC(f.year, f.month - 1, f.day, f.hour % 24, f.minute, f.second) - t;
      }
      function londonMidnight(y, mo, d) {
        var guess = Date.UTC(y, mo - 1, d);          // day overflow is handled for us
        return guess - londonOffsetMs(guess - londonOffsetMs(guess));
      }

      /* End of the day after the match's matchday: 23:59:59.999 UK, expressed as
         the millisecond before midnight opening matchday+2. Saturday's games,
         any kick-off, close just shy of Monday morning. */
      function voteCloseAt(m) {
        var md = bstDateOf(koOf(m));
        if (!md) return null;
        var p = md.split('-');
        return new Date(londonMidnight(Number(p[0]), Number(p[1]), Number(p[2]) + 2) - 1);
      }
      function votableMatch() {
        var now = simNow();
        var reg = state.registration;
        if (!reg) return null;
        /* 'opening' counts as findable, not just 'voting'. A fan whose match has
           just finished should see it sitting there saying "open shortly" — if
           only 'voting' qualified, the widget would tell them no match is open
           and name their NEXT fixture, minutes after the one they watched. */
        var open = (state.allMatches || []).filter(function (m) {
          var st = stateOf(m, now);
          return isOwnTeamMatch(m, reg) && (st === 'voting' || st === 'opening');
        }).sort(function (a, b) { return dateOf(b).localeCompare(dateOf(a)); });
        return open[0] || null;
      }
      // The club's next fixtures, so "come back soon" can say when. Postponed
      // ties are excluded — their listed date is the one that didn't happen.
      // Returns [] at the end of the season, which the card handles.
      function nextFixtures(limit) {
        var now = simNow();
        var reg = state.registration;
        if (!reg) return [];
        return (state.allMatches || []).filter(function (m) {
          var ko = koOf(m);
          if (!isOwnTeamMatch(m, reg) || !ko || ko.getTime() <= now.getTime()) return false;
          return periodOf(m) !== 'postponed';
        }).sort(function (a, b) {
      ...
      function stateOf(m, now) {
        ...
        /* The server's gate, not ours. motm-windows/<matchId> is written by the
           leaderboard job once a match has actually finished, and the rules refuse
           any nomination without it — so full time in the NLS feed is NOT enough
           to offer voting.

           The job runs every quarter of an hour and GitHub's scheduler is
           best-effort, so there is a real gap after full time where this widget
           would happily take a nomination the database then rejects. The fan would
           see a raw PERMISSION_DENIED for something we told them was open. Read
           the same node the rule reads and there is no gap to get wrong. */
        /* Could not read the server's gate at all. Fall back to full time, as
           this did before v2.17. The rule is still the arbiter — a write it
           refuses says so in plain words — and locking every fan out because one
           read failed is a worse trade than offering something the server might
           refuse. Failing closed is right for the RULE, not for the UI. */
        if (state.windowsFailed) {
          var fallbackClose = voteCloseAt(m);
          return (fallbackClose && now < fallbackClose) ? 'voting' : 'closed';
        }
        if (state.windows === null) return 'opening';        // not loaded yet
        var close = state.windows[m.id];
        if (close == null) return 'opening';                 // finished, job not caught up
        return (now < close) ? 'voting' : 'closed';
      }
      // Season actually in force, in priority order:
      //   1. ?season=YYYY — explicit, always wins
      //   2. the SIM clock, when frozen — winding back to a pre-1 Jul 2026 date
      //      should show you that season's fixtures, not this one's
      //   3. clubs-meta seasons.current
      //   4. derived from the real clock
      function activeSeasonId() {
        var forced = seasonOverride();
        if (forced) return forced;
        if (state.sim.fixed) return deriveSeasonId(state.sim.fixed);
        return state.seasonId || deriveSeasonId(new Date());
      }
      ...
      function ensurePlayers(matchId) {
        ...
        fetch(API_BASE + '/matches/' + matchId, { credentials:'omit' })
          ...
          .then(function (json) {
            ...
            // Build a per-player event record across BOTH teams first. Own goals
            // are filed under the team that BENEFITS, not the team the scorer
            // plays for, so scanning per-team would attach them to the wrong
            // player — hence one pass over everything, keyed by playerID.
            // Events are deduped by eventID: NLS sometimes logs a correction as a
            // second copy of the same event (seen on substitutions).
            var evOf = {};
            function evFor(pid) {
              return evOf[pid] || (evOf[pid] = { goals: [], cards: [], on: '', off: '', seen: {} });
            }
            function minuteOf(ev) {
              return ev.formattedEventTime || (ev.eventMinute != null ? ev.eventMinute + "'" : '');
            }
            teams.forEach(function (team) {
              var evs = team.events || {};
              (evs.goals || []).forEach(function (ev) {
                var g = ev && ev.goalEvents; if (!g || !g.playerID) return;
                var rec = evFor(g.playerID), k = 'g' + (g.eventID || ev.eventID || minuteOf(ev));
                if (rec.seen[k]) return; rec.seen[k] = true;
                var t = String(g.goalType || '').toLowerCase();
                rec.goals.push({
                  kind: t.indexOf('own') !== -1 ? 'own' : (t.indexOf('pen') !== -1 ? 'pen' : 'goal'),
                  minute: minuteOf(ev)
                });
              });
              (evs.bookings || []).forEach(function (ev) {
                var b = ev && ev.bookingEvents; if (!b || !b.playerID) return;
                var rec = evFor(b.playerID), k = 'b' + (b.eventID || ev.eventID || minuteOf(ev));
                if (rec.seen[k]) return; rec.seen[k] = true;
                var ct = String(b.cardType || b.card || '').toLowerCase();
                // A second yellow IS a sending off, so record both cards — that is
                // what happened, and it reads correctly on the row.
                if (ct.indexOf('second') !== -1) {
                  rec.cards.push({ card: 'yellow', minute: minuteOf(ev) });
                  rec.cards.push({ card: 'red',    minute: minuteOf(ev) });
                } else {
                  rec.cards.push({ card: ct.indexOf('red') !== -1 ? 'red' : 'yellow', minute: minuteOf(ev) });
                }
              });
              (evs.subs || []).forEach(function (ev) {
                var s = ev && ev.substitutionEvents; if (!s) return;
                if (s.subOnID)  evFor(s.subOnID).on   = minuteOf(ev);
                if (s.subOffID) evFor(s.subOffID).off = minuteOf(ev);
              });
            });
            ...
            teams.forEach(function (team) {
              ...
              var subsUsed = ((team.players && team.players.Sub) || []).filter(function (p) {
                return evOf[p.playerID] && evOf[p.playerID].on;
              });
              ...
              all.forEach(function (p) {
                ...
                list.push({
                  ...
                  events:    evOf[p.playerID] || { goals: [], cards: [], on: '', off: '' },
                  teamId:    teamID,
                  teamName:  teamName
                });
              });
      ...
      // ---------- match event icons ----------
      // Inline SVG, no emoji: emoji render differently on every platform, cannot
      // take a brand colour, and at 11px turn to mush. currentColor lets each
      // icon inherit the semantic colour set on its wrapper.
      var ICON = {
        // Goal and cards are the club-site icon set: assets/icons/ball.svg and
        // yellow-card/red-card.svg. Inlined rather than referenced, because a
        // cross-origin <use href> does not resolve from the CMS — the files are
        // the source of truth and this is a copy, so re-copy if they change.
        // The ball carries its shape in solid mass, which is why it survives
        // down to 11px where a drawn-outline ball collapses to a dot.
        goal: '<svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true">' +
                '<path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="' +
                  'M5.32024 7.05356C5.19953 7.03364 5.07587 7.01324 4.94791 ' +
                  '6.99245C6.08871 5.43953 7.5217 4.37242 9.35119 3.76011C9.33189 ' +
                  '3.87307 9.31343 3.98265 9.29535 4.08989C9.25592 4.32385 ' +
                  '9.21838 4.54665 9.17819 4.76902C9.11879 5.09743 9.15989 ' +
                  '5.15173 9.47779 5.19563C10.3048 5.30983 11.12 5.46673 11.8694 ' +
                  '5.86484C11.9402 5.90234 12.0664 5.90084 12.1373 ' +
                  '5.86263C12.8713 5.46663 13.6734 5.31603 14.4844 ' +
                  '5.20003C14.8464 5.14833 14.8804 5.11313 14.8152 ' +
                  '4.74162C14.7782 4.53139 14.7424 4.32097 14.7044 ' +
                  '4.09745C14.6858 3.98852 14.6668 3.87647 14.6468 ' +
                  '3.75981C16.4746 4.37102 17.9126 5.43523 19.052 6.99065C18.925 ' +
                  '7.01142 18.8017 7.03189 18.6808 7.05195C18.4066 7.09744 ' +
                  '18.1451 7.14083 17.8831 7.18095C17.524 7.23595 17.4852 7.29105 ' +
                  '17.5543 7.64456C17.713 8.45677 17.8458 9.27028 17.7095 ' +
                  '10.1025C17.6976 10.175 17.739 10.2852 17.7943 10.3354C18.4234 ' +
                  '10.9072 18.8344 11.6306 19.2244 12.369C19.3765 12.6571 19.4344 ' +
                  '12.6777 19.7237 12.5149C19.9214 12.4036 20.1187 12.2914 ' +
                  '20.3289 12.1719C20.4335 12.1125 20.5413 12.0512 20.6539 ' +
                  '11.9873C20.6363 13.8773 20.0907 15.542 18.9933 17.082C18.9523 ' +
                  '16.9958 18.9125 16.912 18.8736 16.8301C18.7698 16.6114 18.6721 ' +
                  '16.4056 18.5739 16.2001C18.4303 15.8996 18.3725 15.8848 ' +
                  '18.0825 16.035C17.3409 16.419 16.583 16.7577 15.7432 ' +
                  '16.8718C15.6686 16.8819 15.5708 16.9446 15.5387 ' +
                  '17.0098C15.1635 17.7737 14.578 18.3658 13.9767 18.9453C13.7353 ' +
                  '19.1779 13.7345 19.2354 13.9791 19.481C14.1402 19.6428 14.2978 ' +
                  '19.8082 14.4615 19.98C14.5367 20.0588 14.6131 20.139 14.6917 ' +
                  '20.2208C12.893 20.8008 11.1075 20.7981 9.30409 20.2212C9.38262 ' +
                  '20.1397 9.45908 20.0601 9.53421 19.9819C9.70478 19.8042 ' +
                  '9.86852 19.6337 10.0341 19.465C10.2632 19.2317 10.2612 19.1758 ' +
                  '10.0216 18.9449C9.42029 18.3653 8.83619 17.7722 8.4586 ' +
                  '17.0101C8.4263 16.9447 8.329 16.8814 8.2545 16.8715C7.3987 ' +
                  '16.758 6.631 16.403 5.87471 16.0164C5.63661 15.8947 5.56241 ' +
                  '15.9131 5.44131 16.1587C5.39204 16.2585 5.34549 16.3596 ' +
                  '5.29894 16.4607C5.2771 16.5082 5.25525 16.5556 5.23311 ' +
                  '16.603L5.02391 17.0483C4.18461 16.3387 3.20802 13.3914 3.36482 ' +
                  '11.9995C3.41833 12.0296 3.47016 12.0586 3.52084 12.087C3.62076 ' +
                  '12.143 3.71625 12.1965 3.81151 12.2504C3.87353 12.2855 3.93528 ' +
                  '12.3212 3.99704 12.3569C4.11561 12.4253 4.23419 12.4938 ' +
                  '4.35471 12.5586C4.55331 12.6653 4.63021 12.6276 4.74721 ' +
                  '12.4307C4.77125 12.3903 4.79527 12.3498 4.8193 12.3093C5.09474 ' +
                  '11.8453 5.37042 11.3808 5.66751 10.9308C5.74801 10.8089 ' +
                  '5.85231 10.7031 5.95686 10.597C6.03075 10.522 6.10477 10.4469 ' +
                  '6.17061 10.3659C6.23361 10.2883 6.29871 10.168 6.28501 ' +
                  '10.0791C6.15761 9.25358 6.28881 8.44737 6.4444 7.64256C6.5121 ' +
                  '7.29225 6.4737 7.23465 6.11441 7.18075C5.85443 7.14171 5.59498 ' +
                  '7.09889 5.32024 7.05356ZM2.00034 11.9229C1.95494 17.4652 ' +
                  '6.40763 21.9594 11.9838 21.9997C17.4213 22.039 21.9497 17.5776 ' +
                  '21.9996 12.1319C22.0505 6.56466 17.6261 2.0499 12.0705 ' +
                  '2.0004C6.56223 1.9513 2.04574 6.40166 2.00034 11.9229ZM11.51 ' +
                  '14.8567C11.6713 14.8572 11.8327 14.8577 11.994 ' +
                  '14.8577V14.8564C12.1552 14.8564 12.3164 14.856 12.4776 ' +
                  '14.8555C12.882 14.8544 13.2865 14.8533 13.6909 14.8592C13.9169 ' +
                  '14.8625 14.0526 14.7774 14.1232 14.5596C14.4712 13.4855 ' +
                  '14.8231 12.4127 15.1789 11.3411C15.2423 11.1501 15.2001 ' +
                  '11.0106 15.0388 10.8953C14.1072 10.2284 13.1758 9.56134 ' +
                  '12.2467 8.89103C12.0782 8.76933 11.9269 8.76993 11.758 ' +
                  '8.89193C10.8351 9.55824 9.90971 10.2211 8.98342 ' +
                  '10.8825C8.81642 11.0016 8.76032 11.139 8.82802 11.3427C9.18172 ' +
                  '12.4072 9.52921 13.4739 9.87531 14.541C9.94681 14.7616 10.0802 ' +
                  '14.8646 10.3194 14.8606C10.7161 14.8541 11.113 14.8554 11.51 ' +
                  '14.8567Z' +
                '"/>' +
              '</svg>',
        // The card files are the same rounded rectangle, differing only in a
        // hardcoded #FED800 / #FE0000. Those two values move to CSS as
        // --card-yellow / --card-red so one shape serves both and the colour
        // sits with the rest of the palette.
        card: '<svg viewBox="0 0 24 24" width="13" height="13" aria-hidden="true">' +
                '<rect x="4" y="0" width="16" height="24" rx="2" fill="currentColor"/>' +
              '</svg>',
        on:   '<svg viewBox="0 0 12 12" width="10" height="10" aria-hidden="true">' +
                '<path d="M6 1.5 11 8.5H1z" fill="currentColor"/></svg>',
        off:  '<svg viewBox="0 0 12 12" width="10" height="10" aria-hidden="true">' +
                '<path d="M6 10.5 1 3.5h10z" fill="currentColor"/></svg>'
      };

      // The strip of what happened to a player. Minutes are in the tooltip for
      // goals and cards — showing every one turns the row into a timeline — but
      // visible for substitutions, where the minute tells you how long they
      // actually played, which is the thing you are judging.
      function eventStripHTML(ev) {
        if (!ev) return '';
        var out = '';

        var real = ev.goals.filter(function (g) { return g.kind !== 'own'; });
        var own  = ev.goals.filter(function (g) { return g.kind === 'own'; });
        function balls(list, cls, label) {
          if (!list.length) return '';
          var mins = list.map(function (g) {
            return (g.kind === 'pen' ? 'Penalty ' : '') + (g.minute || '');
          }).join(', ');
          var t = ' title="' + $h(label + (mins ? ' — ' + mins : '')) + '"';
          // Repeat the ball up to three, then collapse to a count so a hat-trick
          // reads at a glance but a five-goal haul does not wrap the row.
          if (list.length <= 3) {
            return '<span class="nlsm__ev ' + cls + '"' + t + '>' +
              new Array(list.length + 1).join(ICON.goal) + '</span>';
          }
          return '<span class="nlsm__ev ' + cls + '"' + t + '>' + ICON.goal +
                 '<span class="nlsm__evx">&times;' + list.length + '</span></span>';
        }
        out += balls(real, 'is-goal', real.length === 1 ? 'Goal' : real.length + ' goals');
        out += balls(own, 'is-own', own.length === 1 ? 'Own goal' : own.length + ' own goals');

        ev.cards.forEach(function (c) {
          out += '<span class="nlsm__ev is-' + c.card + '" title="' +
            $h((c.card === 'red' ? 'Red card' : 'Yellow card') + (c.minute ? ' — ' + c.minute : '')) +
            '">' + ICON.card + '</span>';
        });

        if (ev.on) {
          out += '<span class="nlsm__ev is-on" title="' + $h('On ' + ev.on) + '">' +
            ICON.on + '<span class="nlsm__evmin">' + $h(ev.on) + '</span></span>';
        }
        if (ev.off) {
          out += '<span class="nlsm__ev is-off" title="' + $h('Off ' + ev.off) + '">' +
            ICON.off + '<span class="nlsm__evmin">' + $h(ev.off) + '</span></span>';
        }
        return out ? '<span class="nlsm__evs">' + out + '</span>' : '';
      }

      // GK / DF / MF / FW from the NLS position string. Anything unrecognised
      // returns '' rather than a guess — a wrong position badge is worse than none.
      function posAbbr(position) {
        var p = String(position || '').toLowerCase();
        if (p.indexOf('goal') === 0 || p === 'gk') return 'GK';
        if (p.indexOf('defend') === 0 || p === 'df') return 'DF';
        if (p.indexOf('midfield') === 0 || p === 'mf') return 'MF';
        if (p.indexOf('strik') === 0 || p.indexOf('forward') === 0 || p.indexOf('attack') === 0) return 'FW';
        return '';
      }
      ...
      // ---------- hero ----------
      // Club-branded header. No greeting (the fan knows their own name) and no
      // repeat of "Team of the Week" (the sponsor bar above already says it) —
      // the crest and the club's own colour do the identifying, which makes the
      // widget feel like it belongs to that club rather than to us.
      function renderHero() {
        var r = state.registration;
        if (!r) { hero.className = ''; hero.innerHTML = ''; return; }
        var pal = getTeamPalette(r.teamId);
        var tx  = onClubColour(pal.primary, pal.secondary);
        // Some clubs' primary IS white (Boreham Wood). Without an outline the
        // band would disappear into the page, so pale colours get a border.
        var pale = luminanceOf(pal.primary) > 0.85 ? ' is-pale' : '';
        // Naming the fan here means their name in the quote consent line below
        // is not the first time they see it, which read as jarring.
        var who = state.user && (state.user.fullName || state.user.forename);
        hero.className = 'nlsm__hero';
        hero.innerHTML =
          '<div class="nlsm__clubhero' + pale + '" style="--club-bg:' + $h(pal.primary) + ';--club-tx:' + $h(tx) + ';">' +
            (r.crestUrl
              ? '<img class="nlsm__clubcrest" src="' + $h(r.crestUrl) + '" alt="" ' +
                  'onerror="this.onerror=null;this.style.display=\'none\';">'
              : '') +
            '<span class="nlsm__clubwho">' +
              (who ? '<span class="nlsm__clubfan">' + $h(who) + '</span>' : '') +
              '<span class="nlsm__clubname">' + $h(r.teamName) + '</span>' +
            '</span>' +
          '</div>' +
      ...
      // Shown whenever nothing is open for nominations. Deliberately short: two
      // lines of explanation, then the club's next games. The fixtures block
      // disappears entirely at the end of the season rather than leaving a
      // heading with nothing under it.
      function windowCardHTML() {
        var reg = state.registration;
        var upcoming = nextFixtures(3);
        var list = '';
        if (upcoming.length) {
          list =
            '<div class="nlsm__fixtures">' +
              '<div class="nlsm__fixtureshead">Upcoming games for ' + $h(reg ? reg.teamName : 'your club') + '</div>' +
              upcoming.map(function (m) {
                var a = m.attributes;
                var ko = koOf(m);
                var isHome = a.homeTeam.teamID === reg.teamId;
                var opp = isHome ? a.awayTeam : a.homeTeam;
                return '<div class="nlsm__fixture">' +
                  crestImg(opp.crest, opp.name || opp.shortName || '', 28) +
                  '<span class="nlsm__fixtureopp">' +
                    $h(clubShort(opp.teamID, opp.shortName || opp.name)) +
                  '</span>' +
                  '<span class="nlsm__fixtureha' + (isHome ? ' is-home' : '') + '">' +
                    (isHome ? 'H' : 'A') +
                  '</span>' +
                  '<span class="nlsm__fixturewhen">' +
                    '<span class="nlsm__fixturedate">' + $h(fmtBSTDateShort(ko)) + '</span>' +
                    '<span class="nlsm__fixtureko">' + $h(fmtBST(ko)) + '</span>' +
                  '</span>' +
                '</div>';
              }).join('') +
            '</div>';
        ...
        return '<div class="nlsm__windowcard">' +
          ...
          '<p>Pick your candidates for Team of the Week from full time until the end of the next day.</p>' +
          list +
        '</div>';
      }
      ...
      function rowHTML(m, now) {
        ...
        if (s === 'pre') {
          ...
          /* Finished, but the server has not opened the window yet. Says what is
             happening rather than showing a disabled thing with no explanation —
             the wait is minutes, not an error. */
          meta = '<span class="nlsm__ftlabel">Full time</span>' +
                 '<span class="nlsm__lock">Nominations open shortly</span>';
          ...
          /* Name the deadline while it is far off, count down once it is near.
             The window now runs to a fixed 23:59, so relSoon rendered most of it
             as "in 1d" — a worse answer than "Sunday 23:59" when the time is
             knowable. "Sunday 23:59" is the worse answer at twenty past eleven,
             hence the switch. */
          var msLeft = voteClose - now;
          var closesIn = msLeft > 6 * 3600e3
            ? fmtBSTWeekday(voteClose) + ' 23:59'
            : relSoon(msLeft);
        ...
        // Crest, full club name, score — the result block reads left-to-right
        // like any results list. The mirrored right-alignment belongs to the
        // home column of the picker below, not here.
        function teamLine(t, side) {
          var full = t.name || t.shortName || '';
          return '<div class="nlsm__teamline">' +
            crestImg(t.crest, full) +
            '<span class="nlsm__tname">' + $h(full) + '</span>' +
            scoreCell(side) +
          '</div>';
        }
        ...
        return '<div class="' + rowCls + '">' +
          ...
          (action ? '<div class="nlsm__rowsplit"></div>' + action : '') +
        '</div>';
      }
      ...
      function pickHeroHTML(m, vote, canChange) {
        ...
        if (p) {
          role = (p.status === 'Start')
            ? (p.position || 'Started')
            : ('Substitute' + (p.events && p.events.on ? ' &middot; on ' + $h(p.events.on) : ''));
        }
      ...
      // Relative luminance of a club colour, 0 (black) to 1 (white).
      ...
      // Type on a club's colour uses that club's OWN secondary — it is the
      // pairing they publish, and what the picker's team headers already use.
      // Falls back to a contrast-derived token only when the declared secondary
      // would be illegible on the primary (some clubs' meta pairs two near
      // identical shades), so a bad pair degrades to readable, not invisible.
      function onClubColour(primary, secondary) {
        var lp = luminanceOf(primary), ls = luminanceOf(secondary);
        var ok = /^#[0-9a-fA-F]{6}$/.test(String(secondary || '').trim());
        if (ok && Math.abs(lp - ls) >= 0.35) return secondary;
        return pickTextColor(primary);
      }
      ...
      function pickTextColor(hex) {
        ...
        return luminanceOf(hex) > 0.68 ? 'var(--text)' : 'var(--white)';
      }
      ...
      function pickerHTML(m, draft, hadVote) {
        ...
        function teamCol(team, isHome) {
          var palette = getTeamPalette(team.teamID);
          var ps = list.filter(function (p) { return p.teamId === team.teamID; });
          // Keep NLS's own array order — players.Start[] already arrives as a
          // teamsheet reads (keeper, defence, midfield, attack), and subs after.
          // Do NOT sort by formationPlace: it is a pitch-slot code, not a running
          // 1-11, so sorting on it scrambles the positions. Do not sort by shirt
          // either, which puts whoever wears 2 above the keeper. The only thing
          // enforced here is that starters precede substitutes.
          var order = {};
          ps.forEach(function (p, i) { order[p.playerId] = i; });
          ps.sort(function (x, y) {
            var sx = x.status === 'Start' ? 0 : 1;
            var sy = y.status === 'Start' ? 0 : 1;
            if (sx !== sy) return sx - sy;
            return order[x.playerId] - order[y.playerId];
          });
          var seenSub = false;
          var rows = ps.map(function (p) {
            var isSub = p.status !== 'Start';
            var divider = '';
            if (isSub && !seenSub) { seenSub = true; divider = '<div class="nlsm__subdiv">Substitutes</div>'; }
            // No '#': the number sits in a squad-colour pill, which reads as a
            // shirt number on its own.
            var shirt = p.shirt != null ? String(p.shirt) : '';
            ...
            return divider +
              '<button type="button" class="' + cls + '" ' +
                ...
                eventStripHTML(p.events) +
                '<span class="nlsm__ppos">' + $h(posAbbr(p.position)) + '</span>' +
              '</button>';
          }).join('');
          var teamLabel = team.shortName || team.name || '';
          // Same treatment as the hero: use the club's declared secondary, but
          // fall back where the pair would be illegible, and flag a near-white
          // primary so the header strip and number pills keep an edge.
          var onColour = onClubColour(palette.primary, palette.secondary);
          var paleCol  = luminanceOf(palette.primary) > 0.85 ? ' is-pale' : '';
          return '<div class="nlsm__teamcol' + (isHome ? ' is-home' : '') + paleCol +
                 '" style="--team-primary:' + $h(palette.primary) + ';--team-secondary:' + $h(onColour) + ';">' +
        ...
        return '<div class="nlsm__picker">' +
          ...
          '<div class="nlsm__teampick">' +
            teamCol(a.homeTeam, true) +
            teamCol(a.awayTeam, false) +
          '</div>' +
          '<div class="nlsm__note">' +
            ...
            '<p class="nlsm__noteexample">We pick the best lines for our Team of the Week article — ' +
              'yours could run alongside the player. By filling in this section you consent to ' +
              'being quoted in our Team of the Week article using your name' +
              (state.user && state.user.fullName ? ' (' + $h(state.user.fullName) + ')' : '') + '.</p>' +
      ...
      function saveVote(mid) {
        ...
        if (!m) return;
        var st = stateOf(m, simNow());
        if (st !== 'voting') {
          /* Not open and closed are different answers, and only one of them means
             "you have missed it". */
          showBanner(st === 'opening'
            ? 'Nominations have not opened for this match yet — try again in a few minutes.'
            : 'Nominations have closed for this match.', 'err');
          return;
        }
        ...
        fbDb.ref('motm/' + state.user.id + '/' + mid).set(payload)
          ...
          .catch(function (err) {
            ...
            state.pendingMid = null;
            renderFixtures();
            /* PERMISSION_DENIED here means the window is shut at the server —
               either the job has not opened it yet, or the deadline has passed.
               Neither is something a fan can act on by reading a Firebase error
               code, and both are ordinary rather than broken. */
            var denied = /permission[_ ]denied/i.test(String(err && (err.code || err.message)));
            showBanner(denied
              ? 'Nominations are not open for this match right now. They open just after full time and close at 23:59 the next day.'
              : 'Nomination failed: ' + err.message, 'err');
          });
      }
      ...
      // the UI can render.
      function listenAll() {
        /* Live, not once: the entry for a match that has just finished appears
           mid-session, and a fan sitting on the page at full time should see it
           open without reloading. */
        fbDb.ref('motm-windows').on('value', function (snap) {
          state.windows = snap.val() || {};
          renderFixtures();
        }, function () {
          /* Denied or offline. NOT an empty object — that reads as "the server
             says nothing is open" and locks every fan out of a window the rule
             would have allowed. Flag the failure and let stateOf fall back to
             full time instead. */
          state.windowsFailed = true;
          renderFixtures();
        });
      ...
      function startSignedIn(claims) {
        ...
        state.user = {
          ...
          // Quotes now run under the fan's full name (they consent to that at
          // the point of writing one), so it is shown back to them and stored.
          fullName: ((claims.forename || '') + ' ' + (claims.surname || '')).trim(),
          email:    claims.email    || '',
        ...
        Promise.all([
          ...
          }).then(function () { state.fbAuthed = true; }).then(function () {
            /* The server's nomination window, BEFORE the first paint.
               listenAll() subscribes to this as well, but it runs after
               renderAll() and only answers after a round trip — so without this
               read every finished match rendered "Nominations open shortly" until
               the socket replied, which is exactly what it looked like on the
               morning after matchday one. */
            return fbDb.ref('motm-windows').once('value').then(function (snap) {
              state.windows = snap.val() || {};
            }).catch(function () {
              state.windowsFailed = true;
            });
