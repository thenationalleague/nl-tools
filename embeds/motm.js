    ...
    (function () {
            ...
            teams.forEach(function (team) {
              ...
              all.forEach(function (p) {
                ...
                list.push({
                  ...
                  name:      name,
                  shirt:     p.shirtNumber,
                  // NLS orders the XI by formationPlace (1 = keeper, then out
                  // from the back), which is how a teamsheet actually reads.
                  place:     (typeof p.formationPlace === 'number') ? p.formationPlace : 99,
      ...
      function pickerHTML(m, draft, hadVote) {
        ...
        function teamCol(team, isHome) {
          ...
          // Starters first, in NLS's own teamsheet order (formationPlace: keeper,
          // defence, midfield, attack) rather than by shirt number, which mixes
          // positions up. Substitutes follow, ordered by shirt.
          ps.sort(function (x, y) {
            var sx = x.status === 'Start' ? 0 : 1;
            var sy = y.status === 'Start' ? 0 : 1;
            if (sx !== sy) return sx - sy;
            if (sx === 0 && x.place !== y.place) return x.place - y.place;
