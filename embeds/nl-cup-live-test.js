...
(function () {
  ...
  function mount() {
    ...
    (function () {
      ...
      var mode = String((marker && marker.getAttribute('data-nl-cup-live')) ||
        root.getAttribute('data-mode') || '').trim().toLowerCase();
      var contained = mode === 'contained';

      /* The CMS can only place the snippet inside a content column, below the
         site's own header and carousel, and the page template is not editable
         from there. mode "top" is the way out: the block re-parents itself to
         the front of <body>, so the band opens the page and everything below
         moves down. In flow, not fixed — it pushes, never overlays — which also
         means a position:fixed site header still paints above it; everything in
         normal flow is beaten. Done at mount, before first render, so the page
         never shows the band in one place and then jumps it to another. */
      if (mode === 'top' && marker && document.body && document.body.firstChild !== marker) {
        document.body.insertBefore(marker, document.body.firstChild);
      }

      var dateEl  = document.getElementById('nlcl-date');
      var badgeEl = document.getElementById('nlcl-badge');
      ...
      /* The UK day after a UK day, by calendar arithmetic rather than +24 hours —
         on the 25-hour night the clocks go back, midnight plus a day is still the
         same UK date, and the eve preview would sit that Sunday out. */
      function ukNextYmd(ymd) {
      ...
      /* The band is a "watch this now" band. A tie that is over is not something
         to watch, so once none of today's are still to come or under way the day
         is done — the band hands over to tomorrow's round if there is one
         (pickDay), and otherwise goes back to taking up no room. */
      ...
      /* The repo's short, and PL2 for every academy side whatever NLS called it:
         "Birmingham City U21" from the API prints "Birmingham PL2". The
         competition's name for them is the one fans see everywhere else, so the
         tile no longer echoes the feed's tag (it did until 11/09/2026). */
      function shortFor(name) {
        return clubFor(name).short + (suffixOf(name) ? ' PL2' : '');
      }
      ...
      /* One UTC window wide enough for both UK days in view — a day back for the
         BST midnight stragglers, two forward so that tomorrow's evening kick-offs
         (already tomorrow's UTC day, and reached from midnight tonight) are
         inside it. Filtered on the UK date afterwards: the window is a UTC one
         and the answer is a local one. */
      function apiWindow(now) {
        return {
          from: utcYmd(new Date(now.getTime() - 864e5)) + ' 00:00:00Z',
          to:   utcYmd(new Date(now.getTime() + 2 * 864e5)) + ' 23:59:59Z'
        };
      }

      /* Two days are in view. Today's card wins while it still has a tie to come
         or in play; otherwise tomorrow's round takes the band as the eve preview.
         That is what puts Tuesday's cup night on the front page from Monday
         morning — and, in a round split across two evenings, what hands the band
         to the second night once the first is done. */
      function pickDay(matches, today, tomorrow) {
        var todays = matches.filter(function (m) { return ukYmd(m.ko) === today; });
        if (todays.some(watchable)) return todays;
        return matches.filter(function (m) { return ukYmd(m.ko) === tomorrow; });
      }

      function fetchTies(season) {
        var now = new Date();
        var win = apiWindow(now);
        var url = API + '?seasonID=' + encodeURIComponent(season) +
          '&competitionID=' + COMP_CUP +
          '&from=' + encodeURIComponent(win.from) +
          '&to='   + encodeURIComponent(win.to) +
          '&page.number=1&page.size=100';

        var today = ukYmd(now);
        var tomorrow = ukNextYmd(today);
        return getJson(url, { cache: 'no-store' }).then(function (json) {
          return pickDay((json && json.data || []).map(function (m) {
            var a = m.attributes || {};
      ...
      function open(m, links, now) {
        ...
        /* "Watch from 18:45" is an answer about tonight. On the eve card the only
           honest time is the kick-off, so the line waits for the day itself. */
        var sameDay = ukYmd(m.ko) === ukYmd(new Date(now || Date.now()));
        return {
          url: live ? known : '',
          from: (known && !live && m.state === 'pre' && sameDay) ? ukTime(new Date(m.ko.getTime() - ARM_MS)) : '',
          tag: live ? 'a' : 'div',
      ...
      function render(matches, links, fav) {
        ...
        /* The pill may only say Live on the day itself — a pulsing LIVE thirty
           hours before kick-off is exactly the kind of lie the band is built not
           to tell. On the eve it says Tomorrow, flat. */
        badgeEl.innerHTML = ukYmd(matches[0].ko) === ukYmd(new Date())
          ? '<i class="nlcl__dot" aria-hidden="true"></i>Live'
          : 'Tomorrow';
        root.hidden = false;
      ...
      Promise.all([
        ...
        var last = null;
        function cycle() {
          return fetchTies(season).then(function (matches) {
            last = matches;
        ...
        /* Failing silently is the correct failure here: on all but ~20 days a
