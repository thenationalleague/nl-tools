...
| `nl-utils.js`        | `?v=15`       | `window.NL.*` helpers: `toast`, `ensureAuth`, `formatDate`, `escHtml`, `writeAudit`, `installAuditHook`, `icon`, `clubs`, `clubPicker`, plus identity-data exports `mapStyle.drive`, `positionBands`, `projColours` (canvas/data callers). |
