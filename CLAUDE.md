...
| File                 | Current `?v=` | Role                                                              |
|----------------------|---------------|-------------------------------------------------------------------|
| `nl-brand.css`       | `?v=21`       | Brand tokens, components, layout. Tools must use tokens not hex.  |
| `nl-utils.js`        | `?v=20`       | `window.NL.*` helpers: `toast`, `ensureAuth`, `formatDate`, `escHtml`, `writeAudit`, `installAuditHook`, `icon`, `endpoints`, `clubs` (incl. `crestUrl(name[,'thumb'\|'medium'])`, `wireCrestImg`, `byOpta`), `clubPicker`, `roles` (incl. `norm`, `label`, `realm`), `isClubUser`, `canClubEdit`, plus identity-data exports `mapStyle.drive`, `positionBands`, `projColours` (canvas/data callers). |
